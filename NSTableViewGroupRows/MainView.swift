...
final class MainView: NSViewController {

    var tracks: [Track] = []
    var trackDictionary: Dictionary<String, [Track]> = [:]
    var groups: [String] = []
    ...
    override func viewDidLoad() {
        ...
        tracks = [Track(filename:"track1", duration:"3:45"),
                  Track(filename:"track2", duration:"2:32", artist: "artist1", genre: "rock"),
                  Track(filename:"track3", duration:"4:20", genre: "pop")]
        for x in 1...1000 {
            let track = Track(filename:"track\(x+3)", duration:"3:45", artist: "artist\(Int((x % 20) + 10))", genre: "electronic")
            tracks.append(track)
        }
    ...
    private func groupTracks (_ tracks: [Track], by type: GroupType) -> Dictionary<String, [Track]> {
...
extension MainView: NSOutlineViewDataSource {
...
extension MainView: NSOutlineViewDelegate {
    ...
    func outlineView(_ outlineView: NSOutlineView, viewFor tableColumn: NSTableColumn?, item: Any) -> NSView? {
        ...
        if let title = item as? String {
            let groupCell = outlineView.makeView(withIdentifier: NSUserInterfaceItemIdentifier(rawValue: "GroupCell"), owner:self) as! NSTableCellView
            ...
            let itemCell = outlineView.makeView(withIdentifier: NSUserInterfaceItemIdentifier(rawValue: "ItemCell"), owner:self) as! NSTableCellView
